...
// and every miss runs the full PDF typesetting pipeline. Cap the request rate as
// a speed-bump against a render-flood, on top of the edge caching below.
const rateLimited = createRateLimiter({ limit: 30, windowMs: 60_000 });

// Serves the worked sample report as a real .pdf — the primary deliverable a
// paid matter would download (report + AI-Disclosure Appendix + data→opinion
// mapping), professionally typeset. Built from fictional sample data and marked
// as such inside the file; it is not a real expert report and must never be
// presented as one.
const SAMPLE_NOTICE =
...
export async function GET(request: Request) {
  ...
  return new Response(new Uint8Array(buf), {
    ...
    headers: {
      ...
      "Content-Disposition": `attachment; filename="Disclosed-${def.kind}-sample.pdf"`,
